...
export interface DashPayModulePlugin {
    ...
    openOnlineForm(options: { strUri: string }): Promise<{ strUri: string }>;
    getSerial(options: { value: string }): Promise<{ value: string }>;
    print(options: { printString: string,receiptLogo: string, EXTRA_ORIGINATING_URI: string, NewActivityLaunchOption: boolean }): Promise<{ results: any[] }>;
